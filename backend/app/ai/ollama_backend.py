...
def _ask_budget(think: bool) -> int:
    ...
    # (간결한 답변 유도 — 장황함 방지).
    return 3072 if think else 350


def _coach_prompt(context: str, question: str, *, beginner: bool) -> str:
    # 차분한 코치 — 예측·매수매도 조언 금지(가짜 확신 ❌), 맥락·원리로 안심.
    # 인사말·서두 없이 곧바로 핵심부터, 간결하게. [맥락]의 실제 수치를 근거로 활용.
    tone = "초보도 이해하게 전문용어를 풀어서 " if beginner else ""
    return (
        "당신은 초보 장기투자자를 돕는 차분한 한국어 투자 코치입니다.\n"
        "규칙:\n"
        "- 가격 예측이나 매수/매도 시점 조언은 하지 않습니다"
        "('오른다'/'사라/팔아라' 금지). 일반 원리·역사적 경향·맥락으로만 설명합니다.\n"
        "- 인사말·자기소개·사과·감사 같은 서두 없이 곧바로 핵심부터 답합니다.\n"
        f"- {tone}간결하게: 보통 3~5문장 이내, 필요하면 짧은 불릿. "
        "군더더기·반복 금지.\n"
        "- [맥락]에 종목의 실제 수치가 있으면 그 숫자를 근거로 구체적으로 "
        "설명합니다(없는 값은 지어내지 않음).\n"
        "- 장기·적립 관점을 존중하고 불안을 키우지 않습니다.\n\n"
        f"[맥락]\n{context}\n\n[질문]\n{question}"
    )
