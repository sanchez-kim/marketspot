...
- **포트폴리오** — 보유종목·수량·평단 입력(수동) → 실시간 평가액·손익·비중. 시세 없는 종목은 정직하게 제외.
- **토스증권 연동** — 앱키/시크릿만 넣으면 계좌·보유종목·거래내역을 읽기 전용으로 동기화(주문/매매는 하지 않음). 앱이 계산한 보유수량과 토스 실잔고가 다르면 **숨기지 않고 드리프트로 표시**. 한국 종목 시세 보조에도 사용.
...
**데이터 소스(무료):** 시세·차트·뉴스·기본정보·일정 = yfinance(키 불필요, 미국 ~15분 지연) · 공시 = SEC EDGAR(키 불필요) / DART(키 필요) · 심볼검색 = Yahoo · 계좌 연동·한국 시세 보조 = 토스증권 Open API(키 필요) · AI = 로컬 Ollama.
...
- **토스증권 앱키/시크릿은 실제 증권 계좌 접근 권한**이다 — 다른 무료 API 키보다 훨씬 민감하게 다룰 것. 읽기 전용(계좌 조회·시세)만 쓰며 주문 기능은 없다.
...
- **토스증권 연동으로 새로 발견되는 한국 종목의 거래소(코스피/코스닥) 판별은 아직 미확정** — 토스 API가 접미사 없는 코드로 응답을 주는데, 어느 거래소인지 구분할 필드가 있는지 실보유종목으로 검증 전이라 보수적으로 보류 중(자세한 내용은 [STATUS.md](./STATUS.md)).
